<div>
    <div class="flex items-center gap-4 mb-6">
        ...
        <div class="flex-1">
            <flux:heading level="1" size="xl">{{ $endQuiz ? 'Edit' : 'Create' }} End Quiz</flux:heading>
    ...
    <div class="rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 p-6">
        <form wire:submit="save" class="space-y-6">
            <flux:field>
                <flux:label>Video Content</flux:label>
                <flux:select wire:model="contentId" required>
                    <flux:select.option value="">Select a video content</flux:select.option>
                    @foreach ($availableContents as $content)
                        <flux:select.option value="{{ $content['id'] }}">
                            {{ $content['title'] }}
                        </flux:select.option>
                    @endforeach
                </flux:select>
            ...
            <div class="flex justify-end gap-2">
                ...
                <flux:button type="submit" variant="primary">
                    {{ $endQuiz ? 'Update' : 'Create' }} End Quiz
                </flux:button>
            </div>
        </form>
    </div>
